...
# implement BFS to find room with a ?
# visited = set()


def bfs(starting_room):
    '''
    Find the shortest path from current room(deadend)
    to closest room with '?'
    '''
    ...
    while q.size() > 0:
        ...
        if v not in visited:
            ...
            # print(f"BFS v in visited: {graph[v]}")
            for room in graph[v]:
                if graph[v][room] == '?':
                    # IF SO, RETURN PATH
                    return path
            ...
            # APPEND THE NEIGHBOR TO THE BACK
                copy.append(graph[v][neighbor])
                q.enqueue(copy)
        # if nothing is found return none
    return None

# pick a unexplored direction from current room
# travel to unexplored direction
# when you reach a room with no unexplored paths
# use BFS to find shortest distance to an exit '?'
# convert path from BFS as a list of roomIDs to cardinal directions path(route)


def determine_path(route):
    '''
    Takes list of rooms from bfs and
    converts to directions
    '''

    # from current room id

    # The cheeky way:
    # This FAILS hard at scale...
    # x = get length of route
    # route_len = len(route) - 1
    # reversed = []

    # if route_len > 1:
    #     print(f"ROUTE LENGTH {route_len}")
    #     # slice off the last x of traversal path
    #     temp_traversal = traversalPath[-route_len:]
    #     print(f"TRAVERSAL SLICE __ {temp_traversal}")
    #     # reverse them
    #     for direction in temp_traversal:
    #         print(f"direction in loop ** {direction}")
    #         reversed.append(player.currentRoom.reverseRoomDirection(direction))
    #     # return that cheeky array
    ...
    # loop over graph see if key value (room id) matches array index from BFS
    # route_arr = route
    # print(f"current room: {player.currentRoom.id}")
    # print(f"this is route[0]: {route[0]}")
    breadcrumbs = []

    # rebuild directions to unexplored exit
    # while len(route) > 0:
    for room in route:
        # print(f"START __ for key room {room} ROUTE: {route}")
        # for key direction in KVP of route[0]
        for direction in graph[route[0]]:
            # print(
            #     f"for direction: {direction} in graph[route[0]]: {route[0]}: {graph[route[0]]}")
            # print(
            #     f"if room: {room} == {graph[route[0]]}:{direction} > {graph[route[0]][direction]} append {direction} to breadcrumbs")
            # check current room for matching room number, if match send along breadcrumb to move character
            # if all unexplored rooms == 0 again, pass through BFS and repeat until fresh nodes found
            if room == graph[route[0]][direction]:
                breadcrumbs.append(direction)
        # print(f"breadcrumbs {breadcrumbs}")
        # route.pop(0)
        # print(f"route pop: {route}")
    ...
    # route = [2, 1, 0]
    # print(f"DT CURRENT ROOM _____________>>> {player.currentRoom.id}")
    # fake_route = [2, 1, 0]
    # return fake_route
...
# movement algo
while True:
    # Get current room directions from graph
    current_room = graph[player.currentRoom.id]
    # print(f"curren room graph ________ START: {player.currentRoom.id}:{current_room}"
    #       )
    # set unexplored_rooms to empty array
    unexplored_rooms = []
    # print(f"room exits: {player.currentRoom.getExitsString()}")

    # loop through current room directions add to unexplored
    for new_room in current_room:
        # print(f"new_room: {current_room[new_room]}")
        if current_room[new_room] == '?':
            unexplored_rooms.append(new_room)
            # print(f"unexplored rooms not ?: {unexplored_rooms}")

    if len(unexplored_rooms) > 0:
        random.shuffle(unexplored_rooms)
        # print(f"suffled rooms: {unexplored_rooms}")

        # declare next direction from shuffled list
        next_direction = unexplored_rooms[0]
        # print(f"next direction: {next_direction}")
        # print(f"unexplored rooms: {unexplored_rooms}")

        # save current room as previous room
        prev_room = graph[player.currentRoom.id]
        prev_id = player.currentRoom.id

        # add direction traveled to traversalPath
        traversalPath.append(next_direction)
        # print(f"SUPER SENSITIVE UNEXPLORED ROOM{unexplored_rooms}")
        # print(f"traversal path: {traversalPath}")

        # travel in a direction from unexplored rooms
        player.travel(next_direction)

        # set newly explored rooms not in graph directions to '?'
        rooms_to_add = dict()

        if player.currentRoom.id not in graph:
            for direction in player.currentRoom.getExits():
                rooms_to_add.update({direction: '?'})
                # print(f"rooms to add dict: {rooms_to_add}")

            # log moved-into room's exits into graph
            graph[player.currentRoom.id] = rooms_to_add
            # print(f"updated graph: {graph}")
            # print(f"room exits: {player.currentRoom.getExitsString()}")

        # update previous room graph with new room graph
        prev_room[next_direction] = player.currentRoom.id
        # print(f"prev_room: {prev_room}")
        # print(f"updated graph 2: {graph}")

        # update new room direction with prev room id
        graph[player.currentRoom.id][player.currentRoom.reverseRoomDirection(
            next_direction)] = prev_id
        # print(f"previous ID: {prev_id}")
        # print(f"updated graph AFTER travel: {graph}")

    # if all contiguous rooms have been explored
    # pass current room id into BFS and find way back to room with ?
    else:
        # use BFS to find shortest path from current room to room with '?'
        # print(
        #     f"ENDING ROOM PASSED TO BFS: {player.currentRoom.id}")
        new_path = bfs(player.currentRoom.id)
        print(f"PATH RETURNED FROM BFS: {new_path}")

        # if len(new_path) > 1:
        if new_path is not None:
            # pass path into path function to convert to directions
            # print(f"new path from bfs is none trigger")
            directions = determine_path(new_path)
            print(f"path from clean up _______>>> {directions}")

            # pass reverse direction to traversal path
            for direction in directions:
                # print(f"_____direction to append___: {direction}")
                traversalPath.append(direction)
                # print(
                # f"_____traversalPath after append___: {traversalPath}")
                player.travel(direction)
                # print(
                # f"_____PLAYER__ current room after travel: {player.currentRoom.id}")

        if new_path == None:
            print(f"__**__**__**BREAK TRIGGERED**__**__**__")
            break
